...
- **区域查询**：在 Web Viewer 地图上用鼠标框选一个区域，即可查询哪些日期在该区域内有轨迹点，并列出各日期的起止时间与点数。
...
4. 命令行方式：
   ...
   ```
4. 脚本会直接在手机上通过 `adb shell sqlite3` 查询数据，按需返回结果，**不再一次性把整个数据库拉到电脑**，首次打开更快；随后自动打开本地网页 `http://127.0.0.1:8765/`。

功能：

- 地图散点展示、按时间段着色
- 日历热力图
- 日期范围筛选，支持「上一周 / 下一周 / 上一年 / 下一年」快捷跳转
- 轨迹列表，点击日期查看详情
- 区域查询：在地图上框选矩形范围，查询该区域内出现过的日期及起止时间
- 点击点位查看详情
